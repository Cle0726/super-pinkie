...
import { MediaError, prepareMediaBatch } from "../workspace/media.js";
...
function mapError(error) {
    if (error instanceof WorkspaceError || error instanceof MediaError)
...
export function createMcpServer(ctx) {
    ...
    server.registerTool("analyze_media", {
        title: "Analyze selected project media",
        description: `Analyze only the explicitly named project images or videos. Images are sent as inline visual content; videos stay local and only evenly-spaced JPEG keyframes plus technical metadata are sent. Never scan a directory for media or upload a raw video. Calling this tool sends the selected visual content to the requesting ChatGPT account. ${UNTRUSTED_NOTE}`,
        inputSchema: {
            paths: z.array(z.string()).min(1).max(8).describe("Exact workspace-relative image or video paths selected by the user"),
        ...
        if (denied)
            return denied;
        try {
            const result = await prepareMediaBatch(workspace, args.paths, { videoFrames: args.video_frames });
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({
                            ...
                            media: result.items,
                            sentPreviewBytes: result.sentBytes,
                            note: "Only the exact named media was provided. Original workspace files were not modified; raw video and audio were not uploaded.",
                        }, null, 2),
                    },
                    ...result.visuals,
                ],
