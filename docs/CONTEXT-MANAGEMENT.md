...
整理不再追求“越短越好”：四个主模式使用 safeguard 分段整理，至少保留最近 8 轮原文，并对摘要做质量复查；派对空间的交接记录会明确保留用户硬约束、项目根目录、文件变更、命令与工具结果、错误原文、验证结果和下一步。压缩前还会把可续工检查点写入各工作区的 `memory/context/active.md`。原始聊天记录仍在，整理只改变下一轮送给模型的上下文。
