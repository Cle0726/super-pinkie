...
自动压缩前，CLE Kk 会在本机状态目录建立一份短期“续工交接包”：当前目标、当前工作区、`memory/context/active.md` 的检查点、档位进度，以及最近的真实工具结果。它不是聊天记录副本，也不会写入其他模式的记忆。

压缩完成后的下一轮会把交接包作为系统上下文重新交给模型：先核对文件和工具现场，已经成功的写入、发布、下载等步骤不能重做，然后从最早未完成步骤接着执行、验证并正常回复用户。交接只注入一次；正常完成后会清掉，过期也会自动删除，所以不会污染之后的新任务。

如果内核明确报出“自动压缩已完成但无法恢复本轮”，CLE Kk 最多自动续接两次。普通“最新输入本身太大 / context overflow”不会硬重试——那样只会再次塞爆上下文；它会保留可见错误，等待分段输入或换更大模型。所有续接都使用原会话和原模型，不会重复向用户发送看门狗控制消息。
