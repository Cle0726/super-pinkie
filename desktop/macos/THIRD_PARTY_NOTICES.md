...
超級碧琪.app includes the following runtimes so the local application can start without a separate command-line installation:
...
The application does not bundle the user's API keys, provider credentials, chat history, project files, or `~/.openclaw/openclaw.json`.
