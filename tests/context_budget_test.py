...
class ContextBudgetTests(unittest.TestCase):
    ...
    def test_provider_limits_are_not_guessed_from_names(self):
        ...
        self.assertEqual(9600,a['threshold']);self.assertEqual(76800,b['threshold'])
        unknown=budget['model_budget']('other/same',cfg,self.home)
        self.assertEqual('conservative-fallback',unknown['source']);self.assertEqual(128000,unknown['window'])
        self.assertEqual(76800,unknown['threshold'])
        self.assertEqual(44800,unknown['keepRecent'])
        self.assertEqual(44800,unknown['target'])
    ...
    def test_existing_policy_is_migrated_to_safe_hysteresis_without_losing_other_values(self):
        ...
        self.assertEqual(.65,saved['triggerRatio']);self.assertEqual(.45,saved['targetRatio'])
        self.assertEqual(.45,saved['keepRecentRatio']);self.assertEqual(5,len(saved['adaptiveTiers']));self.assertEqual('keep',saved['custom'])
    ...
    def test_codex_metadata_and_explicit_override(self):
        ...
        self.assertEqual(123500,budget['model_budget']('codex-cli/model-a',{},self.home)['threshold'])
        policy=self.home/'Library/Application Support/SuperPinkie/context-policy.json';policy.parent.mkdir(parents=True)
        policy.write_text(json.dumps({'modelLimits':{'codex-cli/model-a':64000}}))
        self.assertEqual(38400,budget['model_budget']('codex-cli/model-a',{},self.home)['threshold'])
    ...
    def test_setup_preserves_personas_auth_and_explicit_values_and_is_idempotent(self):
        ...
        self.assertEqual(64000,provider['models'][0]['contextWindow']);self.assertEqual(128000,provider['models'][1]['contextWindow'])
        self.assertNotIn('reserveTokens',after['agents']['defaults']['compaction']);self.assertNotIn('reserveTokensFloor',after['agents']['defaults']['compaction'])
    ...
    def test_agent_cap_does_not_overwrite_provider_capacity_and_override_is_effective(self):
        ...
        self.assertEqual(9600,budget['model_budget']('a/known',saved,self.home)['threshold'])
        ...
        self.assertEqual(7200,budget['model_budget']('a/known',saved,self.home)['threshold'])
